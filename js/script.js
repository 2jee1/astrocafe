$(document).ready(function () { // html 태그를 다 읽었다면 문서가 준비되면
    ...
    const dthWrap = gnbLi.children('.depth_wrap')
    const depth2 = dthWrap.find('a')

    console.log('const dthWrap = ' , dthWrap)
    console.log('const depth2 = ' , depth2)
    ...
    /* 퀵메뉴 */
    const quickMn = $('.quick_menu')
    const quickBtn = quickMn.children('.quick_button')

    quickBtn.on('click', function () {
        quickMn.toggleClass('close')

        const condition1 = $(this).parent().hasClass('close')
        const condition2 = $(this).children('span').text();

        console.log('버튼 안에 span 태그 안에 들어있는 글자는?', $(this).children('span').text())
        console.log('버튼 안에 span 태그 안에 들어있는 글자가 퀵메뉴 닫기 입니까?', $(this).children('span').text() == '퀵메뉴 닫기')

        if (condition2) {
            $(this).children('span').text('퀵메뉴 열기')
        } else {
            $(this).children('span').text('퀵메뉴 닫기')
        }
    })
